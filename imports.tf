...
import {
  ...
  id = "" # Stack slug, e.g. "ghalactic"
}

import {
  to = grafana_cloud_access_policy.workflow_observability
  id = "" # "{region}:{policy_id}"
}

# Note: access policy token is NOT imported — Terraform will recreate it
# and push the new value to OTLP_PASSWORD. The old token will stop working.
# Service account and SA token are also new resources, no import needed.

# GitHub org variables and secret

import {
  to = github_actions_organization_variable.otlp_endpoint
  id = "" # Variable name, e.g. "OTLP_ENDPOINT"
}

import {
  to = github_actions_organization_variable.otlp_username
  id = "" # Variable name, e.g. "OTLP_USERNAME"
}

import {
  to = github_actions_organization_secret.otlp_password
  id = "" # Secret name, e.g. "OTLP_PASSWORD"
}
